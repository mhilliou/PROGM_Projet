...
class _Game2State extends State<Game2> {
  ...
  void _avancerPion() {
    setState(() {
      _pionPosition +=
          10.0; // Vous pouvez ajuster la valeur d'incrémentation ici
      ...
      if (_pionPosition >= _ligneArrivee) {
        ...
        var score = 0;
        if (_tempsEcoule <= Duration(seconds: 4)) {
          score = 3;
        } else if (_tempsEcoule <= Duration(seconds: 6)) {
          score = 2;
        } else if (_tempsEcoule <= Duration(seconds: 8)) {
          score = 1;
        } else {
          score = 0;
        }
            ...
            : Navigator.push(
                ...
                MaterialPageRoute(
                    builder: (context) => EndGamePageAleatoire(
                        score: widget.scoreJeu + 1, nbJeu: widget.nbJeu)),
              );
    ...
    showDialog(
      ...
      builder: (BuildContext context) {
        ...
        return AlertDialog(
          title: Text('Victoire !'),
          content: Text(
              'Vous avez atteint la ligne d\'arrivée en $tempsFormate secondes.'),
          actions: [
            TextButton(
              child: Text('OK'),
              onPressed: () {
